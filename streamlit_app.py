...
# Load ALL Excel files with column name handling
@st.cache_data
def load_all_excel_files():
    try:
        ...
        if 'PROVIDER_APPROVING_NAME' in df.columns:
            df['Technician'] = df['PROVIDER_APPROVING_NAME'].str.extract(r'(\w+)\s')[0].str.capitalize()
        else:
            st.error("Column 'PROVIDER_APPROVING_NAME' not found")
            return None
        ...
        elif 'Most Recent Approval Date (CST)' in df.columns:
            date_col = 'Most Recent Approval Date (CST)'
        else:
        ...
        df['duration_sameday'] = df.groupby('Technician')[date_col].diff().dt.total_seconds()
        
        st.write(f"✅ Data processed successfully!")
        st.write(f"Technicians: {sorted(df['Technician'].unique())}")
